trigger trigger_CustomerCommits on Customer_Commitments__c (after insert, after update) {
    ...
    try {
        for (Integer i=0; i < Trigger.new.size(); i++) {
            Customer_Commitments__c b = Trigger.new[i];
            Customer_Commitments__c b_old = Trigger.old == null ? new Customer_Commitments__c() : Trigger.old[i];
